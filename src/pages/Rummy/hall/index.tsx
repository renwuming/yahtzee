...
export default function Index() {
  ...
  const [hallGameList, setHallGameList] = useState<Rummy.RummyGameData[]>([]);
  const [hallPageNum, setHallPageNum] = useState<number>(0);
  const [myGameList, setMyGameList] = useState<Rummy.RummyGameData[]>([]);
  ...
  return (
    <View className="rummy-hall">
        ...
        <AtTabsPane current={tabIndex} index={0}>
          ...
          >
            {hallGameList.map((game, index) => {
              return (
                <GameItem game={game} index={index} gameType="Rummy"></GameItem>
              );
            })}
            {hallPageEnd ? (
              <AtDivider
      ...
      <View className="fab-btn history">
        <AtFab
          onClick={() => {
            navigateTo("Rummy", `history/index`);
          }}
        >
